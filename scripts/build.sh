...
configure() {
   ...
   echo "SDK ${DEVROOT}/usr/bin/"

   local PREFIX="${BUILD_DIR}/${CURL_VERSION}-${OS}-${ARCH}"

   export PATH="${DEVROOT}/usr/bin/:${PATH}"
   export CFLAGS="-arch ${ARCH} -pipe -Os -gdwarf-2 -isysroot $SDK -m$TYPE-version-min=$IPHONEOS_DEPLOYMENT_VERSION -fembed-bitcode"
   ${SRC_DIR}/configure --disable-shared --enable-static --enable-ipv6 --host="${HOST}" --prefix=${PREFIX} --with-secure-transport
}
